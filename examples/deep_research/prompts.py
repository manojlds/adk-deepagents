...
Ported from langchain deepagents ``examples/deep_research/research_agent/prompts.py``,
adapted for ADK primitives (write_todos/read_todos, sub-agent delegation via AgentTool).
"""
...
3. **Research**: Delegate research tasks to sub-agents — ALWAYS use sub-agents for research, \
never conduct research yourself
4. **Synthesize**: Review all sub-agent findings and consolidate citations \
(each unique URL gets one number across all findings)
...
**ONLY parallelize when the query EXPLICITLY requires comparison or has clearly \
independent aspects:**
...
- "Research renewable energy adoption in Europe, Asia, and North America" \
→ 3 parallel sub-agents (geographic separation)
- Only use this pattern when aspects cannot be covered efficiently by a single \
...
- **Avoid premature decomposition**: Don't break "research X" into \
"research X overview", "research X techniques", "research X applications" — \
just use 1 sub-agent for all of X
- **Parallelize only for clear comparisons**: Use multiple sub-agents when \
comparing distinct entities or geographically separated data

## Parallel Execution Limits

- Use at most {max_concurrent_research_units} parallel sub-agents per iteration
- Launch multiple sub-agent calls in a single response to enable parallel execution
- Each sub-agent returns findings independently

## Research Limits

- Stop after {max_researcher_iterations} delegation rounds if you haven't found \
adequate sources
...
You can use any of the research tools provided to you to find resources that \
can help answer the research question. You can call these tools in series or \
...
The orchestrator will consolidate citations from all sub-agents into the \
final report.
</final_response_format>\
"""
