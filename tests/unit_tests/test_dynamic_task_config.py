"""Unit tests for dynamic task config helpers."""
...
def test_build_dynamic_task_config_defaults_to_wait_policy(monkeypatch) -> None:
    _clear_dynamic_task_env(monkeypatch)

    config = build_dynamic_task_config()
...
def test_build_dynamic_task_config_reads_env_overrides(monkeypatch) -> None:
    _clear_dynamic_task_env(monkeypatch)
...
def test_build_dynamic_task_config_ignores_invalid_env_values(monkeypatch) -> None:
    _clear_dynamic_task_env(monkeypatch)
...
def test_build_dynamic_task_config_enables_temporal_from_env(monkeypatch) -> None:
    _clear_dynamic_task_env(monkeypatch)
...
def test_build_dynamic_task_config_reads_temporal_env_overrides(monkeypatch) -> None:
    _clear_dynamic_task_env(monkeypatch)
...
def test_build_dynamic_task_config_ignores_invalid_temporal_numeric_env(monkeypatch) -> None:
    _clear_dynamic_task_env(monkeypatch)
    monkeypatch.setenv("ADK_DEEPAGENTS_TEMPORAL_TARGET_HOST", "127.0.0.1:7233")
