...
For the dynamic `task` implementation (`config=DeepAgentConfig(delegation_mode="dynamic")`) and runtime
...
delegation is enabled (`DeepAgentConfig(delegation_mode="static")` or `"both"`, including the
